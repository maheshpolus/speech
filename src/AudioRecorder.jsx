...
const AudioRecorder = () => {
  ...
  const [watchStarted, setStopWatchStarted] = useState(false);
  ...
  const startRecording = async () => {
    ...
    setStopWatchStarted(!watchStarted);

    const media = new MediaRecorder(stream, {
      type: "audio/webm; codecs=opus",
      // type: "audio/wav;codecs=pcm",
      audioBitsPerSecond: 16 * 1000, // 16-bit sample size
      numberOfAudioChannels: 1, // Single channel
  ...
  const stopRecording = () => {
    ...
    mediaRecorder.current.onstop = async () => {
      ...
      const response = await speechService.saveSpeechToServer(data);
  ...
  return (
    <div>
      <div className="card">
        ...
        <div>
          {permission && recordingStatus === "inactive" ? (
            <button
              ...
              className="button slide start-button"
              id="scenarioStartButton"
            >
              <svg
          ...
          {recordingStatus === "recording" ? (
            <button
              ...
              className="button stop stop-button"
              id="scenarioStartButton"
            >
              <svg
                onClick={stopRecording}
        ...
        {isStopWatchActive && (
          <Stopwatch start={stopWatch} watchstarted={watchStarted} />
        )}
      </div>
    </div>
  );
...
function WebmToWavConverter() {
  const [audioBlob, setAudioBlob] = useState(null);

  const handleConvert = async (event) => {
    const webmBlob = event.target.files[0];
    ...
    mediaSource.addEventListener("sourceopen", async () => {
      const mediaSourceBuffer = mediaSource.addSourceBuffer(
        "audio/webm; codecs=opus"
      );
      ...
      mediaSourceBuffer.appendBuffer(await webmBlob.arrayBuffer());
    });

    const wavBlob = new Blob([audioBlob], { type: "audio/wav" });
    // Do something with the WAV blob
  };

  return (
    <div>
      <input type="file" accept="audio/webm" onChange={handleConvert} />
    </div>
  );
}
